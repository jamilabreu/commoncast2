class UsersController < ApplicationController
  def index
    if params[:recent]
      ...
      @posts = Post.plusminus_tally.joins(:communities).where("posts.created_at >= ?", DateTime.now - 1.week).where(communities: {id: current_user.community_ids}).select('posts.*, COUNT(distinct communities.id) AS community_count').having('COUNT(distinct communities.id) >= 2').group('posts.id').order('community_count DESC, plusminus_tally DESC')
